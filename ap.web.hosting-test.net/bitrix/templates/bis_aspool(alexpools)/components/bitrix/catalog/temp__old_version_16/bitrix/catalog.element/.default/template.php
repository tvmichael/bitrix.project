...
$arItemIDs = array(
	...
	'SLIDER_MODAL_ID' => $strMainID.'_slider_modal_id',
...
if ( $USER->IsAdmin() && $USER->GetID() == 126 ) { 
	// -------------------------------------------------------------------------------------------------------------------------
	//echo '<h1>template.php</h1>';
	//echo '<div class="col-md-12"><pre>'; 
	//echo $arResult['ID'].'<br><br>';
	//print_r($arResult['DETAIL_PICTURE']); 
	//print_r($arResult['MORE_PHOTO']); 
	//print_r($arResult); 
	//print_r($arParams['DETAIL_PICTURE_MODE']); 
	//echo '</pre></div>'; 
};
?>
...
<div class="bx_item_detail <? echo $templateData['TEMPLATE_CLASS']; ?>" id="<? echo $arItemIDs['ID']; ?>">
	...
	<div class="bx_item_container">
		...
		<div class="bx_lt">
					...
					<div class="bx_bigimages_imgcontainer">
						...
						if ('Y' == $arParams['SHOW_DISCOUNT_PERCENT'])
						{
							if (!isset($arResult['OFFERS']) || empty($arResult['OFFERS']))
							{
								...
								{
						?>
							<div class="bx_stick_disc right bottom" id="<? echo $arItemIDs['DISCOUNT_PICT_ID'] ?>"><? echo -$arResult['MIN_PRICE']['DISCOUNT_DIFF_PERCENT']; ?>%
							</div>
						<?
								}
							}
							...
							{
						?>
							<div class="bx_stick_disc right bottom" id="<? echo $arItemIDs['DISCOUNT_PICT_ID'] ?>" style="display: none;">
							</div>
						<?
							}
						}
						?>
						<div class="bx_stick average left top" <?= (empty($arResult['LABEL'])? 'style="display:none;"' : '' ) ?> id="<? echo $arItemIDs['STICKER_ID'] ?>" title="<? echo $arResult['LABEL_VALUE']; ?>"><? echo $arResult['LABEL_VALUE']; ?>					
						</div>
					</div>
				</div>
				<?
				if ($arResult['SHOW_SLIDER'])
				{
					...
					<div class="<? echo $strClass; ?>" id="<? echo $arItemIDs['SLIDER_CONT_ID']; ?>">
						<div class="bx_slider_scroller_container">
							...
							<div class="bx_slide_left" id="<? echo $arItemIDs['SLIDER_LEFT']; ?>" style="<? echo $strSlideStyle; ?>">
							</div>
							<div class="bx_slide_right" id="<? echo $arItemIDs['SLIDER_RIGHT']; ?>" style="<? echo $strSlideStyle; ?>">
							</div>
						</div>
					</div>
					<?
						...
						foreach ($arResult['OFFERS'] as $key => $arOneOffer)
						{
							...
							<div class="<? echo $strClass; ?>" id="<? echo $arItemIDs['SLIDER_CONT_OF_ID'].$arOneOffer['ID']; ?>" style="display: <? echo $strVisible; ?>;">
								<div class="bx_slider_scroller_container">
									...
									<div class="bx_slide_left" id="<? echo $arItemIDs['SLIDER_LEFT_OF_ID'].$arOneOffer['ID'] ?>" style="<? echo $strSlideStyle; ?>" data-value="<? echo $arOneOffer['ID']; ?>">	
									</div>
									<div class="bx_slide_right" id="<? echo $arItemIDs['SLIDER_RIGHT_OF_ID'].$arOneOffer['ID'] ?>" style="<? echo $strSlideStyle; ?>" data-value="<? echo $arOneOffer['ID']; ?>">
									</div>
								</div>
							</div>
							<?
			...
			<? if ($arParams['DETAIL_PICTURE_MODE'] == 'GALLERY') {?>
				<!-- Modal -->
				<div class="bx-modal-slider-1">			
					<div class="modal fade" id="<?= $arItemIDs['SLIDER_MODAL_ID'];?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" data-backdrop="false">
					      		...
					      		<div class="modal-body">					        
									<div id="carousel-example-generic" class="carousel slide" data-interval="false" data-ride="carousel">
									  ...
									  <div class="carousel-inner" role="listbox">
									  	...
									  	<?
									  	foreach ($arResult['MORE_PHOTO'] as $id => $arOnePhoto)
										{ 
											?>
											<div class="item text-center <? if ($id == 0) echo 'active';?>">
									      		<img class="bx-modal-slider-1-img" src="<?=$arOnePhoto['SRC'];?>" alt="">
									   		</div>
											<?									  		
									  	}
									  	?>
...
if (isset($arResult['OFFERS']) && !empty($arResult['OFFERS']))
{
	...
	$arJSParams = array(
		'CONFIG' => array(
			...
			'MAIN_PICTURE_MODE' => $arParams['DETAIL_PICTURE_MODE'], // update- 13-01-18 тип работи слайдера  -  POPUP, MAGNIFIER
		...
		'SLIDER_CONFIG' => array(
			'SLIDER_MODAL_ID' => $arItemIDs['SLIDER_MODAL_ID'],
			'SLIDER_MODAL_MAIN_PICRURE_ID' => $arItemIDs['PICT'],
		),
	);
	if ($arParams['DISPLAY_COMPARE'])
	{
