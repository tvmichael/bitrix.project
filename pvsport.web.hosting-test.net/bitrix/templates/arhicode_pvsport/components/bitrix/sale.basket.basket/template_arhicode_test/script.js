console.log('JS: BasketPoolQuantity start');
...
function updateBasketTable(basketItemId, res)
{
	...
	BX.ajax({
		...
		method: 'POST',
		data: res.BASKET_DATA.ITEMS.AnDelCanBuy,
		dataType: 'json',
		onsuccess: function(result) {
			if(!!result.ITEMS)
			{
				...
				for(id in result.ITEMS) 
				{
						...
						{
							var bonus_one = BX.create('DIV', {html: '+ ' + item.ADD_BONUS_UNIT + ' ' + result.TEXT.TEXT_BONUS_FOR_ITEM});
							
							BX.cleanNode(BX("bonus_unit_" + item.BASKET_ITEM_ID));
							
							if(parseFloat(item.ADD_BONUS_UNIT) > 0)
								BX("bonus_unit_" + item.BASKET_ITEM_ID).appendChild(bonus_one);
						}
						
						if(BX("bonus_sum_" + item.BASKET_ITEM_ID))
						{
							var bonus_sum = BX.create('DIV', {html: '+ ' + item.ADD_BONUS + ' ' + result.TEXT.TEXT_BONUS_FOR_ITEM});
							
							BX.cleanNode(BX("bonus_sum_" + item.BASKET_ITEM_ID));
				...
				if(BX("all_bonus"))
				{
					var bonus_all = BX.create('DIV', {html: '+ ' + result.ALL_BONUS + ' ' + result.TEXT.TEXT_BONUS_FOR_ITEM});
					
					BX.cleanNode(BX("all_bonus"));
					
					if(parseFloat(result.ALL_BONUS) > 0)
						BX("all_bonus").appendChild(bonus_all);
				}
			}
	...
	if (!!res.BASKET_DATA)
	{
		...
		if (BX('allSum_FORMATED'))
		{
			...
			compareSumWidthPrice(res.BASKET_DATA.allSum);
		}
...
// update-26-12-2017 - summ 5000р
function compareSumWidthPrice(totalPprice){
	var sum = 5000; // amount for comparison
	var messageContainer = $('#progress-bar-message-container');
	var progressBar = messageContainer[0].getElementsByClassName('progress-bar')[0];
	var amounNeed = messageContainer[0].getElementsByClassName('shipping-threshold-amount-need')[0];
	//console.log(progressBar);
	if (totalPprice < sum )
	{
		$(messageContainer).show();		
		$(progressBar).css('width', (totalPprice*100/sum)+'%');
		amounNeed.innerHTML = sum - totalPprice;
	} 
	else $(messageContainer).hide();
}
