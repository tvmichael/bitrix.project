...
<div id="basket_items_list">
			...
			<thead>
				<tr>
					...
					foreach ($arResult["GRID"]["HEADERS"] as $id => $arHeader):
						...
						if ($arHeader["id"] == "NAME"):
						?>					
							<td id="col2"><?=GetMessage("PICTURE");?></td>

							<td class="item" id="col2<?/* =getColumnId($arHeader) */?>">
							
						<?
						elseif ($arHeader["id"] == "PRICE"):
						?>
							<td class="price" id="col2">
						<?
						else:
						?>
							<td class="custom" id="col2">
						<?
						endif;
						?>
					...
					<tr id="<?=$arItem["ID"]?>">
						...
						foreach ($arResult["GRID"]["HEADERS"] as $id => $arHeader):
							...
							?>
								...
								<td class="cart-item-name" id="col1" style="white-space: normal;">
									<?if (strlen($arItem["DETAIL_PAGE_URL"]) > 0):?><a href="<?=$arItem["DETAIL_PAGE_URL"] ?>"><?endif;?>
											<?=$arItem["NAME"]?>
										<?if (strlen($arItem["DETAIL_PAGE_URL"]) > 0):?></a><?endif;?>
	...
	<div class="bx_ordercart_order_pay">
		...
		<div>
				<div class="cont-buy">
				<a  class="arhi_basc" href="/catalog/" >Продолжить покупки</a>
				</div>
				<div class="bx_ordercart_order_pay_center">
					...
					<?endif;?>
					<a class="arhi_basc" href="javascript:void(0)" onclick="checkOut();" class="checkout"><?=GetMessage("SALE_ORDER")?></a>
				</div>
		</div>
	</div>
</div>
