...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		{		
			...
			$("#soa-property-3").mask("+38(000) 000-00-00");
		...
		getDeliveryLocationInput: function(node)
		{
			...
			currentProperty.NAME = BX.message('bx-soa-custom-label-currentProperty-NAME-City');
			}
		...
		editPaySystemItems: function(paySystemNode)
		{
			...
			var paySystemItemsContainer = BX.create('DIV', {props: {className: 'col-sm-5 bx-soa-pp-item-container'}}),
				paySystemItemNode, i;
		...
		editPaySystemInfo: function(paySystemNode)
		{
			...
			var paySystemInfoContainer = BX.create('DIV', {
					props: {
						className: (this.result.PAY_SYSTEM.length == 0 ? 'col-sm-12' : 'col-sm-7') + ' bx-soa-pp-desc-container'
					}
				}),
			...
			if (currentPaySystem.ID == 9){ 
			currentPaySystem.DESCRIPTION = BX.message('MESS_NUMBER_CARD') +
					' ' + BX.util.htmlspecialchars(BX.message('bx-soa-pp-company-desc-PaySystem-transfer'));
			} else if (currentPaySystem.ID == 10){ 
			currentPaySystem.DESCRIPTION = BX.util.htmlspecialchars(BX.message('bx-soa-pp-company-desc-PaySystem-Cash'));
			} else if (currentPaySystem.ID == 1){ 
			currentPaySystem.DESCRIPTION = BX.util.htmlspecialchars(BX.message('bx-soa-pp-company-desc-PaySystem-Cash-on-pickup'));
			} else if (currentPaySystem.ID == 8){ 
			currentPaySystem.DESCRIPTION = BX.util.htmlspecialchars(BX.message('bx-soa-pp-company-desc-PaySystem-LIQPAY'));
			}
				title = BX.create('DIV', {
		...
		editDeliveryItems: function(deliveryNode)
		{
			...
			var deliveryItemsContainer = BX.create('DIV', {props: {className: 'col-sm-5 bx-soa-pp-item-container'}}),
				deliveryItemNode, k;
		...
		editDeliveryInfo: function(deliveryNode)
		{
			...
			var deliveryInfoContainer = BX.create('DIV', {props: {className: 'col-sm-7 bx-soa-pp-desc-container'}}),
			...
			if (currentDelivery.ID == 32) currentDelivery.DESCRIPTION = BX.message('bx-soa-pp-company-desc-Nova-Poshta');
			if (currentDelivery.ID == 3) currentDelivery.DESCRIPTION = BX.message('bx-soa-pp-company-desc-Samoviviz');
			title = BX.create('DIV', {
				props: {className: 'bx-soa-pp-company-block'},
				children: [
		...
		createDeliveryItem: function(item)
		{
			...
			{
				if (item.ID == 32) item.OWN_NAME = BX.message('bx-soa-pp-company-title-Nova-Poshta');
				if (item.ID == 3) item.OWN_NAME = BX.message('bx-soa-pp-company-title-Samoviviz');
				title = BX.create('DIV', {
		...
		editFadeDeliveryContent: function(node)
		{
			...
			{
				if (selectedDelivery.ID == 32) selectedDelivery.NAME = BX.message('bx-soa-pp-company-title-Nova-Poshta');
				if (selectedDelivery.ID == 3) selectedDelivery.NAME = BX.message('bx-soa-pp-company-title-Samoviviz');
