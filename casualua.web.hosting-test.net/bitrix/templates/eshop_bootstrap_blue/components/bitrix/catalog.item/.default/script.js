(function (window){
	...
	window.JCCatalogItem = function (arParams)
	{
		...
		if (typeof arParams === 'object')
		{
			...
			// update-	
			this.blockData.recommendList = arParams.BLOCKS_DATA.RECOMMEND_LIST;
			this.blockData.infoText = arParams.BLOCKS_DATA.INFO_TEXT;
	...
	window.JCCatalogItem.prototype = {
		init: function()
		{
			...
			if(/(android|bb\d+|meego).+mobile|avantgo|bada\/|blackberry|blazer|compal|elaine|fennec|hiptop|iemobile|ip(hone|od)|ipad|iris|kindle|Android|Silk|lge |maemo|midp|mmp|netfront|opera m(ob|in)i|palm( os)?|phone|p(ixi|re)\/|plucker|pocket|psp|series(4|6)0|symbian|treo|up\.(browser|link)|vodafone|wap|windows (ce|phone)|xda|xiino/i.test(navigator.userAgent) 
  			...
  			if(this.isMobile){
				...
				this.blockData.btnI = $('i',this.blockData.btn)[0];
				$(this.blockData.btnI).removeClass('fa-angle-right');
				$(this.blockData.btnI).addClass('fa-angle-down');
				$(this.blockData.btnI).css('left', '4px');
				$(this.blockData.btnI).css('top', '0px');

				this.blockData.btn.href='javascript: void(0)';
				BX.bind(this.blockData.btn, 'click', BX.proxy(this.isMobileContainerShowHide, this));				
				window.addEventListener('resize', function(e){					
					var x = parseInt(getComputedStyle(this.blockData.block).width);
     				x = x/2 - 12;     		
		...
		isMobileContainerShowHide: function(e){
			...
			{
				$(this.blockData.btnI).removeClass('fa-angle-up');
				$(this.blockData.btnI).addClass('fa-angle-down');
				$(this.blockData.btnI).css('left', '4px');
				$(this.blockData.btnI).css('top', '0px');
			...
			{
				$(this.blockData.btnI).removeClass('fa-angle-down');
				$(this.blockData.btnI).addClass('fa-angle-up');
				$(this.blockData.btnI).css('left', '4px');
				$(this.blockData.btnI).css('top', '-3px');
		...
		hoverOn: function(event)
		{
			...
			if (this.blockData.modalOpen) return;

			this.panelOrderProduct();
			this.blockData.btn.style.display = 'block';
		...
		sendToBasket: function()
		{
			...
			BX.ajax({
				...
				onsuccess: BX.proxy(this.basketResultV2, this) // update- 18-02-09
			});
		},
