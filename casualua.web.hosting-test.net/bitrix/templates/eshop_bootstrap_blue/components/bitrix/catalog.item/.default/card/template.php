...
<div class="product-item">
	...
	<a  href="<?=$item['DETAIL_PAGE_URL']?>" id="<?=$itemIds['PRODUCT_BLOCKS_BTN_ID'];?>" class="cs-product-blocks-order-container-btn">
				...
				switch ($blockName)
				{
					case 'price': ?>
						<div class="product-item-info-container product-item-price-container product-item-hidden" data-entity="price-block">
							...
							{
								if ($price['RATIO_PRICE'] !== $price['RATIO_BASE_PRICE']) $show_price_col_i = '6';
								?>
								<div class="col-xs-6 col-sm-6 col-md-6 text-center" <?=($price['RATIO_PRICE'] >= $price['RATIO_BASE_PRICE'] ? 'style="display: none;"' : '')?>>
								...
								if ($actualItem['CAN_BUY'])
								{
									...
									*/?>
									<div id="<?=$itemIds['BASKET_ACTIONS']?>" class="cs-button-container" style="display: <?=($actualItem['CAN_BUY'] ? '' : 'none')?>;">
										...
										<? if ($USER->IsAdmin() ) 
										{?>
											<button class="bx_big bx_bt_button buy_one_click_popup cs-button-discount" id="<?=$itemIds['BTN_BUY_ONECLICK'];?>" data-offer-id="0">
