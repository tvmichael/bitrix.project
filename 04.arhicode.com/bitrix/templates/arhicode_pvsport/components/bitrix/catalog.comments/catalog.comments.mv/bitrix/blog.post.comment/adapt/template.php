...
<script>					
    commentObjectTransferJs.editCommentNew = {
        'id1':'0',
        'id2':parseInt('<?=$arParams["ID"];?>')
    };                    
	BX.ready( function(){
		if(BX.viewImageBind)
		{
...
//							dbg showComment?
		...
		if($arResult["is_ajax_post"] != "Y")
		{
			if($arResult["CanUserComment"])
			{
				...
				<!-- update- 18-01-31  add > style="display: none;-->
				<div class="blog-add-comment" style="display: none;">
					<a data-edit-comment-new-id="<?=$arParams["ID"]?>" class="bx_medium bx_bt_button" href="javascript:void(0)" onclick="return editCommentNew('0', <?=$arParams["ID"]?>)">
						<b><?=GetMessage("B_B_MS_ADD_COMMENT")?></b>
					</a><br />

					<!-- update-18-01-30 adapt\template. -->
			...
			if($arResult["CanUserComment"])
			{
				?>
				<!-- update- 18-01-31   add > style="display: none;" -->
				<div id="form_comment_0" style="display: none;">
