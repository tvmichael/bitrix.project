...
?>
<div class="row">
	<div class="<?=($isSidebar ? "osnova-el col-xs-12 col-sm-8 col-sm-push-4 col-md-9 col-md-push-3" : "col-xs-12")?>">
...
?>
	...
	<?if ($isSidebar):?>
	<div class="baner-el hidden-xs col-sm-4 col-sm-pull-8 col-md-3 col-md-pull-9">
...
$arFilter = array(
    ...
    'ACTIVE' => 'Y',
	"PROPERTY_BDETAIL"=>IntVal($ElementID)
);
$iCount = CIBlockElement::GetList(false, $arFilter, array()); 
//echo "кільк ".$iCount; //Если указать в качестве arGroupBy пустой массив, то функция вернет количество элементов CNT по фильтру

	if($iCount>0){
	 global $arrFilter_idElem;
		$arrFilter_idElem=array("PROPERTY_BDETAIL"=>IntVal($ElementID));
	}?>
			 <?$APPLICATION->IncludeComponent(
	"bitrix:news.list", 
	"baners_catalog", 
