										...
										<? if($arParams["SHOW_OFFERS_FIRST_STEP"] == 'Y'): ?>
											...
											<? else: ?>
												...
												<?else:?>
													...
													if(intval($arResult['CURRENT_PRODUCT']['OFFER_ID_MIN_PRICE']) > 0){
														$ar_offers = $arResult['CURRENT_PRODUCT']['OFFERS'][intval($arResult['CURRENT_PRODUCT']['OFFER_ID_MIN_PRICE'])];
													}else{
...
<script>
	var h2oUpdateMask = function () {
		var list_phone_prop = <? echo CUtil::PhpToJSObject($arParams['ID_FIELD_PHONE'], false, true); ?>;	
		$.each(list_phone_prop, function(index, value){
			$("#"+value).mask("<?=$arParams['MASK_PHONE']?>")
		});
	}
</script>