 ...
 */
		...
		// отримуємо номер карточки з властивості замовлень NUMBER_PRIVAT_CARD що привязана до платіжної системи 41
		$db_props = CSaleOrderProps::GetList(
		        array("SORT" => "ASC"),
		        array(
						
					"CODE"=>array("NUMBER_PRIVAT_CARD"),
					"PAYSYSTEM_ID" => 41		
		            )
		    );
		if ($props = $db_props->Fetch())
		{
			if(!isset($_SESSION['NUMBER_CARD'])){
				$_SESSION['NUMBER_CARD'] = $props["DEFAULT_VALUE"];
		
			}

		}
		// end отримуємо номер карточки з властивості замовлень NUMBER_PRIVAT_CARD що привязана до платіжної системи 41
...
else
{
	...
	<script>
		...
		BX.message(<?=CUtil::PhpToJSObject(array('MESS_NUMBER_CARD'=>$_SESSION['NUMBER_CARD']))?>);
